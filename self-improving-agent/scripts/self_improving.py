...
def replace_block(path,ident,transform):
    text=path.read_text(encoding='utf-8',errors='replace'); old=block_for(text,ident)
    if old is None: return False
    new=transform(old).rstrip()+'\n'; atomic_write(path,text.replace(old,new,1)); return True
...
def find_entry(args,ident,prefer_non_public=True):
    ...
    if prefer_non_public:
        non_public=[p for p in found if p.parent not in (PUBLIC,LEGACY/'public')]
        if non_public: return non_public[0]
...
def update(args):
    ...
    with multi_lock(src.parent,PUBLIC):
        source=block_for(src.read_text(encoding='utf-8',errors='replace'),args.id); final=apply(source)
...
def promote(args):
    ...
    with multi_lock(src.parent,PUBLIC):
        source_block=block_for(src.read_text(encoding='utf-8',errors='replace'),args.id)
...
def review(args):
    counts={s:0 for s in VALID_STATES}; promotions={s:0 for s in VALID_PROMOTIONS}; incomplete=[]; duplicate={}; copies={}; seen=set()
    for p in markdown_files(args):
        for ident,block in entries(p.read_text(encoding='utf-8',errors='replace')):
    ...
    for ident,paths in duplicate.items():
        ...
        allowed=same_type and len(active)<=1 and len(public)<=1 and len(legacy)<=1
        if len(unique)>1 and not allowed: expected.append(ident)
    drift=[]
    ...
    print(f'信息不完整：{len(incomplete)} 条；异常重复 ID：{len(expected)} 条；副本漂移：{len(drift)} 条')
    if args.verbose:
...
def parser():
    p=argparse.ArgumentParser(description='Self Improving Agent v3.2')
...
def main():
    ...
    elif a.skill: a.base=LEGACY; a.mode='legacy_skill'
