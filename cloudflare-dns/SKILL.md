...
3. **执行操作**：按需执行 list/create/update/delete；update/delete 前先按 `name + type` 精确列出候选记录 ID，避免误改同名不同类型记录
4. **结果验证**：写操作后重新 `dns list`，核对 type/name/content/proxied/TTL；必要时再用公共 DNS/DoH 查询传播结果
5. **结果展示**：报告记录 ID、变更前后值和验证结果，不输出任何 Token/API Key
