...
version: 2.0.0
compatibility: >
  Requires git and python3. Uses env GITHUB_TOKEN for GitHub API + HTTPS push (non-interactive via GIT_ASKPASS); local Git-only operations do not require a token.
---
...
> 本地 Git 命令默认在当前仓库运行。`init`、`clone`、`pr --upstream ...` 和带 `--repo owner/repo` 的 `gh-*` API 命令可在仓库外运行。
...
|14| `delete-branches --keep <branch> --yes` | 删除除 keep 外的本地/远程分支，并逐项报告 OK/FAIL |
...
|20| `push-main --yes` | 仅在工作区干净且本地 main 不落后时推送 origin/main |
|21| `empty-dir --dir <path> --yes` | 清空仓库内安全相对路径并保留 `.gitkeep`；拒绝 `.git`/越界路径 |
|22| `restore-dir --src <path> --dst <path> --yes` | 覆盖仓库内安全相对路径；拒绝 `.git`/越界路径 |
...
```bash
sh /var/minis/skills/github-sync-helper/scripts/gh_sync.sh empty-dir --dir self-improving-agent --yes
sh /var/minis/skills/github-sync-helper/scripts/gh_sync.sh restore-dir --src /var/minis/skills/self-improving-agent --dst self-improving-agent --yes
sh /var/minis/skills/github-sync-helper/scripts/gh_sync.sh commit --message "restore(self-improving-agent): sync from local"
sh /var/minis/skills/github-sync-helper/scripts/gh_sync.sh push-main --yes
```
...
if [ -d "$repo_dir/.git" ]; then
  cd "$repo_dir"
  test -z "$(git status --porcelain)" || { echo "工作区不干净，先提交或 stash"; exit 1; }
  git fetch --all --prune
  git switch main
  git merge --ff-only origin/main
else
  sh /var/minis/skills/github-sync-helper/scripts/gh_sync.sh clone --url https://github.com/<owner/repo>.git --dir "$repo_dir"
fi
...
- 提交前若报 `Author identity unknown`，可在当前仓库写入：
  - `git config user.name '<GitHub显示名>'`
  - `git config user.email '<login>@users.noreply.github.com'`
- 默认先检查干净工作区，再 `git fetch` + `git merge --ff-only origin/main`；不要用 `reset --hard` 静默丢弃本地修改。
