...
safe_repo_path() {
  [ -n "${ROOT-}" ] || { err "repository root unavailable"; exit 1; }
  case "$1" in ""|/*|..|../*|*/../*|*/..|.git|.git/*) err "unsafe repository path: $1"; exit 2;; esac
  resolved="$(python3 -c 'import os,sys; print(os.path.realpath(os.path.join(sys.argv[1],sys.argv[2])))' "$ROOT" "$1")"
  case "$resolved" in "$ROOT"/*) printf '%s\n' "$resolved";; *) err "path escapes repository: $1"; exit 2;; esac
}
...
gh_api() {
  ...
  M="$method" U="$url" B="$body_json" python3 -c 'import os,sys,json,urllib.request,urllib.error
...
ensure_git_identity() {
  [ -n "$(git config user.name 2>/dev/null || true)" ] && [ -n "$(git config user.email 2>/dev/null || true)" ] && return 0
  name="${GIT_AUTHOR_NAME-}"; email="${GIT_AUTHOR_EMAIL-}"
  if [ -z "$name" ] && [ -n "${GITHUB_TOKEN-}" ]; then
    name="$(python3 "$SCRIPT_DIR/github_api.py" GET /user --field login 2>/dev/null || true)"
  fi
  [ -n "$name" ] || { err "git identity missing: set git user.name/user.email or GIT_AUTHOR_NAME/GIT_AUTHOR_EMAIL"; exit 1; }
  [ -n "$email" ] || email="$name@users.noreply.github.com"
...
# These commands work outside a local repository when --repo/--url is supplied.
case "$cmd" in
  init|clone|pr|gh-*) ROOT="$(repo_root 2>/dev/null || true)"; [ -z "$ROOT" ] || cd "$ROOT" ;;
  "") usage; exit 1 ;;
  *) ROOT="$(repo_root)"; [ -n "$ROOT" ] || { err "not inside a git repo"; exit 1; }; cd "$ROOT" ;;
esac
...
case "$cmd" in
  ...
  stash)
    case "${1-}" in
      ...
      --pop) git stash pop >/dev/null; add_row 1 stash OK popped;;
      --apply) git stash apply >/dev/null; add_row 1 stash OK applied;;
  ...
  delete-branches)
    ...
    for b in $(git branch --format='%(refname:short)'); do
      [ "$b" = "$keep" ] && continue
      if git branch -D "$b" >/dev/null 2>&1; then add_row "$i" "delete local branch" OK "$b"; else add_row "$i" "delete local branch" FAIL "$b"; fi
      i=$((i+1))
    done
    for rb in $(git branch -r --format='%(refname:short)' | grep '^origin/' | sed 's#^origin/##'); do
      case "$rb" in "$keep"|HEAD) continue;; esac
      if git push origin --delete "$rb" >/dev/null 2>&1; then add_row "$i" "delete remote branch" OK "origin/$rb"; else add_row "$i" "delete remote branch" FAIL "origin/$rb"; fi
      i=$((i+1))
    done
    ;;
  ...
  restore-dir)
    ...
    dst="$(safe_repo_path "$dst")"
    [ "$(python3 -c 'import os,sys; print(os.path.realpath(sys.argv[1]))' "$src")" != "$dst" ] || { err "src and dst must differ"; exit 2; }
    ensure_git_identity
  ...
  push-main)
    ...
    ensure_askpass
    ensure_git_identity
    [ -z "$(git status --porcelain)" ] || { err "working tree is dirty; commit or stash first"; exit 2; }
    git show-ref --verify --quiet refs/heads/main || { err "local main branch not found"; exit 1; }
    git checkout main >/dev/null
    git fetch origin main >/dev/null
    counts="$(git rev-list --left-right --count origin/main...main)"
    behind="${counts%%[[:space:]]*}"; ahead="${counts##*[[:space:]]}"
    [ "$behind" = "0" ] || { err "local main is behind/diverged from origin/main; integrate first"; exit 2; }
    git push origin main >/dev/null
    add_row 1 push-main OK "pushed (ahead=$ahead)"
    ;;
...
try:
  ...
  err=e.read().decode("utf-8","ignore"); print("HTTP %s"%e.code+" "+err[:200])')"
...
try:
  ...
  err=e.read().decode("utf-8","ignore"); print("HTTP %s"%e.code+" "+err[:120])')"
...
data=json.dumps(payload).encode("utf-8"); req=urllib.request.Request(f"https://api.github.com/repos/{owner}/{repo}/milestones",data=data,method="POST"); req.add_header("Authorization","Bearer "+tok); req.add_header("Accept","application/vnd.github+json"); req.add_header("User-Agent","minis"); req.add_header("Content-Type","application/json"); resp=json.load(urllib.request.urlopen(req,timeout=30)); print(resp.get("title",""))')"
